...
void Atlas::pack2DTexture(const std::vector<std::string> &paths, bool save) {
  ...
  std::map<std::string, std::unique_ptr<ByteData>> textureBytes;
  ...
  for (int i = 0; i < paths.size(); ++i) {
    ...
    textureBytes[name] = ByteData::MakeCopy(data, image_width * image_height * nrChannels);
  ...
  datas_ = ByteData::MakeWithoutCopy(outData, result_size.w * result_size.h * 4);
}
